...
(function () {
  ...
  async function loadManifest() {
    try {
      ...
      // 匹配当前页面
      const currentPage = window.location.pathname.split("/").pop() || "page_raw.html";
      const pageConfig = manifest.pages?.find(p => p.pageMatch === currentPage);
  ...
  function startSubtitle(videoFile) {
    ...
    function begin() {
      ...
      const interval = (duration * 850) / totalChars;
